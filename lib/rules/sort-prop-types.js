        ...
        if (node.superTypeParameters) {
          ...
          if (component) {
            if (node.superTypeParameters.params[0].type === 'TSTypeReference') {
              ...
              const typeReference = context.getSourceCode().ast.body.find((item) => (item.type === 'TSInterfaceDeclaration' || item.type === 'TSTypeAliasDeclaration') && item.id.name === typeName);
              checkSorted(typeReference.typeAnnotation.members);
