...
module.exports = function propTypesInstructions(context, components, utils) {
  ...
  /**
   * Handle ReturnType<T> in typescript
   * @param {ASTNode} node Node that extends by ReturnType
   * @param {Object} declaredPropTypes
   * @return {Array}
   */
  function returnTypeToPropTypes(node, declaredPropTypes) {
    // ReturnType<T> should always have one parameter
    if (node.typeParameters.params.length === 1) {
      const sourceCode = context.getSourceCode();
      const returnType = node.typeParameters.params[0];
      if (returnType.type === 'TSTypeQuery') {
        const returnTypeFunction = flatMap(sourceCode.ast.body.filter((item) => item.type === 'VariableDeclaration' && item.declarations.find((dec) => dec.id.name === returnType.exprName.name)), (type) => type.declarations).map((dec) => dec.init);
      ...
      } else if (returnType.type === 'TSFunctionType') {
        const returnTypeAnnotation = returnType.returnType.typeAnnotation;
        if (returnTypeAnnotation.type === 'TSTypeReference') {
          const returnTypeAnnotationDeclaration = sourceCode.ast.body.find((item) => item.type === 'TSInterfaceDeclaration' && item.id.name === returnTypeAnnotation.typeName.name);
          if (returnTypeAnnotationDeclaration) {
            return returnTypeAnnotationDeclaration.body.body;
          }
        } else if (returnTypeAnnotation.type === 'TSParenthesizedType') {
          if (returnTypeAnnotation.typeAnnotation.type === 'TSTypeLiteral') {
            return returnTypeAnnotation.typeAnnotation.members;
          }
        } else if (returnTypeAnnotation.type === 'TSTypeLiteral') {
          return returnTypeAnnotation.members;
        }
      }
    }
  }
  ...
  function declarePropTypesForTSTypeAnnotation(propTypes, declaredPropTypes) {
    ...
    let typeName;
    if (propTypes.type === 'TSTypeReference') {
      typeName = propTypes.typeName.name;
    } else if (propTypes.typeAnnotation.type === 'TSTypeReference') {
      typeName = propTypes.typeAnnotation.typeName.name;
    } else if (propTypes.typeAnnotation && propTypes.typeAnnotation.type === 'TSTypeLiteral') {
      foundDeclaredPropertiesList = propTypes.typeAnnotation.members;
    } else {
      // weird cases such as TSTypeFunction
      return true;
    }
    if (foundDeclaredPropertiesList && foundDeclaredPropertiesList.length === 0) {
      const sourceCode = context.getSourceCode();
      if (!typeName) {
        return true;
      }
      // the game here is to find the type declaration in the code
      const candidateTypes = sourceCode.ast.body.filter((item) => item.type === 'VariableDeclaration' && item.kind === 'type');
      ...
      const interfaceDeclarations = sourceCode.ast.body.filter((item) => (item.type === 'TSInterfaceDeclaration' || item.type === 'TSTypeAliasDeclaration') && item.id.name === typeName);
      ...
      if (typeDeclaration) {
        ...
        interfaceDeclarations.forEach((interfaceDeclaration) => {
          if (interfaceDeclaration.type === 'TSInterfaceDeclaration') {
            foundDeclaredPropertiesList = foundDeclaredPropertiesList.concat(interfaceDeclaration.body.body);
          } else if (interfaceDeclaration.type === 'TSTypeAliasDeclaration') {
            // Handle TSIntersectionType like type Props = UserProps & AppProps
            if (interfaceDeclaration.typeAnnotation.type === 'TSIntersectionType') {
              if (Array.isArray(interfaceDeclaration.typeAnnotation.types)) {
                interfaceDeclaration.typeAnnotation.types.forEach((superType) => {
                  if (superType.type === 'TSTypeReference') {
                    declarePropTypesForTSTypeAnnotation(superType, declaredPropTypes);
                  }
                  if (superType.type === 'TSTypeLiteral') {
                    foundDeclaredPropertiesList = foundDeclaredPropertiesList.concat(superType.members);
                  }
                });
              }
              // Handle TSTypeLiteral like type Props = { name: string }
            } else if (interfaceDeclaration.typeAnnotation.type === 'TSTypeLiteral') {
              foundDeclaredPropertiesList = foundDeclaredPropertiesList
                .concat(interfaceDeclaration.typeAnnotation.members);
            }
          }
          // If interface has extension, we will get heritage.
          if (Array.isArray(interfaceDeclaration.extends)) {
            interfaceDeclaration.extends.forEach((h) => {
              const superTypeName = h.expression.name;
              // If interface extends ReturnType like: interface Props extends ReturnType<typeof mapStateToProp>
              if (superTypeName === 'ReturnType') {
                const returnType = returnTypeToPropTypes(h, declaredPropTypes);
                if (Array.isArray(returnType) && returnType.length >= 0) {
                  foundDeclaredPropertiesList = foundDeclaredPropertiesList
                    .concat(returnType);
                }
              } else {
                const superInterfaceDeclaration = sourceCode.ast.body.filter((item) => item.type === 'TSInterfaceDeclaration' && item.id.name === superTypeName);
                superInterfaceDeclaration.forEach((declaration) => {
                  foundDeclaredPropertiesList = foundDeclaredPropertiesList.concat(declaration.body.body);
                });
              }
            });
          }
    ...
    foundDeclaredPropertiesList.forEach((tsInterfaceBody) => {
      if (tsInterfaceBody.type === 'TSPropertySignature' || tsInterfaceBody.type === 'TSMethodSignature') {
        let accessor = 'name';
