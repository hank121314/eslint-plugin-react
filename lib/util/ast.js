 ...
 */
function findAllReturnStatement(node, context) {
  let bodyNodes = [];
  // try to handle utils.isReturningJSX in Components.js traverse 'ReturnStatement, YieldExpression'.
  if (node.type === 'ReturnStatement' || node.type === 'YieldExpression') {
    bodyNodes = [node];
  }
  if (isFunctionType(node)) {
    node = node.value ? node.value : node;
    // If body.type is not blockStatement that means function body might be conciseBody.
    ...
    // Retrieve all return keyword;
    const returnKeywords = tokens.filter((token) => token.type === 'Keyword' && token.value === 'return');
    bodyNodes = returnKeywords.map((keyword) => {
      // Find ReturnStatement node;
      const tokenNode = sourceCode.getNodeByRangeIndex(keyword.range[0]);
      if (tokenNode.type === 'ReturnStatement') {
        let parent = tokenNode;
        while (parent) {
          // If function is the inner function.Like: x.map((iter)=>{return <div></div>});
