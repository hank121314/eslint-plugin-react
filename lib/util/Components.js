...
function componentRule(rule, context) {
  ...
  const utils = {
    ...
    isReturningJSX(ASTnode, strict) {
      const nodesAndProperty = astUtil.findAllReturnStatement(ASTnode, context);
      if (nodesAndProperty.length === 0) {
        return false;
      }
      return nodesAndProperty.map((node) => {
        if (!node) {
          return;
        }
        let property;
    ...
    isReturningJSXOrNull(ASTNode, strict) {
      return utils.isReturningJSX(ASTNode, strict);
    },
  ...
  const detectionInstructions = {
    ...
    'ReturnStatement, YieldExpression'(node) {
